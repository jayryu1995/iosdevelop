...
extension Bundle {
    var ChatApiKey: String {
        guard let file = self.path(forResource: "InfoChatGPT", ofType: "plist") else { return ""}
        guard let resource = NSDictionary(contentsOfFile: file) else { return "" }
        guard let key = resource["API_KEY"] as? String else { fatalError("API_KEY 설정을 확인해주세요.")}
        return key
    }
    ...
    var ChatApiURL: String {
        guard let file = self.path(forResource: "InfoChatGPT", ofType: "plist") else { return ""}
        guard let resource = NSDictionary(contentsOfFile: file) else { return "" }
        guard let key = resource["API_URL"] as? String else { fatalError("API_URL 설정을 확인해주세요.")}
        return key
    }
