...
class InfluenceReportVC: UIViewController {
    ...
    private let businessViewModel = ProposalViewModel()
    ...
    private func setupProfile() {
        if let id = businessId {
            viewModel.getBusinessProfile(member_id: id) { [weak self] result in
    ...
    private func showProposalAlert() {
        ...
        proposalAlertVC.onConfirm = {
            ...
            if let business_id = self.businessId {
                self.businessViewModel.updateProposal(business_id: business_id ) { [weak self] result in
                    switch result {
                    case .success:
