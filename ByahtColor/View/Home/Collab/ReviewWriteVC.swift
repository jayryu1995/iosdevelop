...
class ReviewWriteVC: UIViewController, UIScrollViewDelegate, UINavigationControllerDelegate {
    ...
    private let accessImageLabel: UILabel = {
        let label = UILabel()
        label.text = "Tải ảnh chụp tài khoản"
    ...
    private let imageLabel: UILabel = {
        let label = UILabel()
        label.text = "Tài khoản để liên hệ hướng dẫn nhận sản phẩm"
    ...
    private let snsLabel: UILabel = {
        let label = UILabel()
        label.text = "Nền tảng của ảnh đã đăng"
        label.font = UIFont(name: "Pretendard-SemiBold", size: 14)
        return label
    }()
    ...
    private let nameField: UnderlinedTextField = {
        let field = UnderlinedTextField()
        field.configure(withPlaceholder: "Nhập tên thật", font: UIFont(name: "Pretendard-Regular", size: 14))
        return field
    }()

    private let snsIdLabel: UILabel = {
        let label = UILabel()
        label.text = "Tên tài khoản MXH đã tham gia event"
        label.font = UIFont(name: "Pretendard-SemiBold", size: 14)
        return label
    }()
    ...
    private let snsIdField: UnderlinedTextField = {
        let field = UnderlinedTextField()
        field.configure(withPlaceholder: "Nhập tên tài khoản MXH đã tham gia event", font: UIFont(name: "Pretendard-Regular", size: 14))
        return field
    }()
