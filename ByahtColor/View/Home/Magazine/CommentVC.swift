...
class CommentVC: UIViewController, UITextViewDelegate {
    var contentNo = 0
    var commentList: [MagazineCommentVO] = []
    ...
    private func loadData() {
        let url = "\(Bundle.main.TEST_URL)/magazine/comment/all"
        let parameters: [String: Any] = [
            "magazine_no": contentNo,
            "user_id": User.shared.id ?? ""
        ]

        AF.request(url, method: .post, parameters: parameters)
            .responseDecodable(of: [MagazineCommentVO].self) { response in
    ...
    @objc private func submitButtonTapped() {
        ...
        let url = "\(Bundle.main.TEST_URL)/magazine/comment/update"
        let parameters: [String: Any] = [
            "magazine_no": contentNo,
