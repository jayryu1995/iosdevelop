...
class BoardCell: UITableViewCell, UIScrollViewDelegate {
    ...
    // 데이터 입력
    func configure(with board: ReceiveBoard) {
        ...
        if board.imageList?.isEmpty == false { setImage(imageList: board.imageList!)}
        ...
        AF.request(url, method: .delete, parameters: likeRequestDto, encoder: JSONParameterEncoder.default)
            .response { response in
                ...
                case .success(let data):
                    log(vc: "BoardCell", message: "sendUnlikeRequest request successful")

                case .failure(let error):
                    log(vc: "BoardCell", message: "sendUnlikeRequest Error : \(error)")
                }
            }
    }
    ...
    private func sendLikeRequest(likeRequestDto: BoardLikeDto) {
        ...
        AF.request(url, method: .post, parameters: likeRequestDto, encoder: JSONParameterEncoder.default)
            .response { response in
                switch response.result {
                case .success(let data):
                    log(vc: "BoardCell", message: "sendLikeRequest request successful")

                case .failure(let error):
                    log(vc: "BoardCell", message: "sendLikeRequest Error : \(error)")
                }
            }
    }
}
