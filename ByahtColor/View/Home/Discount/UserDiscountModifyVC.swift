...
class UserDiscountModifyVC: UIViewController, UIScrollViewDelegate, UINavigationControllerDelegate {
    ...
    private func setData() {
        // Text Label
        tv_title.text = discount?.title
        tv_content.text = discount?.content
        ...
        let startDateString = discount?.start_date?.stringToDate()
        let endDateString = discount?.end_date?.stringToDate()
    ...
    private func updateTypeButtonState() {
        for button in tagButtons {
            if let type = discount?.type, type.contains(button.titleLabel?.text ?? "") {
                button.isSelected = true
                button.backgroundColor = .black
                button.setTitleColor(.white, for: .normal)
                selectedType = button.titleLabel?.text ?? ""
            } else {
    ...
    private func setBottomView() {
        ...
        // type 필터 뷰 추가
        let typeFilterView = typeFilterView(tags: ["Beauty", "Clinic"])
        contentView.addSubview(typeFilterView)
        typeFilterView.snp.makeConstraints { make in
            make.top.equalTo(tv_content.snp.bottom).offset(20)
            make.leading.equalToSuperview().offset(20)
            make.trailing.equalTo(view).offset(-20)
        }

        lbl_date.snp.makeConstraints {
            $0.top.equalTo(typeFilterView.snp.bottom).offset(10)
    ...
    private func typeFilterView(tags: [String]) -> UIView {
        let container = createFilterViewContainer(withTitle: "TYPE", tags: tags)
        return container
    }
    ...
    @objc private func uploadButtonTapped() {
        ...
        let url = "\(Bundle.main.TEST_URL)/discount/update/\(discount?.no ?? 0)"
    ...
    private func loadImagesFromUrl() {
        guard let imageUrls = discount?.imageList else { return }

        let group = DispatchGroup()
        imageUrls.forEach { urlString in
            group.enter()
            let url = "\(Bundle.main.TEST_URL)\( urlString )"
            print(url)
...
extension UserDiscountModifyVC: UIImagePickerControllerDelegate, UITextViewDelegate, UITextFieldDelegate {
