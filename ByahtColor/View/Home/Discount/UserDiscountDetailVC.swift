...
class UserDiscountDetailVC: UIViewController, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Discount"
        notification = discount?.notification ?? false
    ...
    private func setView() {
        ...
        // 셀 등록
        tableView.register(UserDiscountDetailCell.self, forCellReuseIdentifier: "UserDiscountDetailCell")
    ...
    @objc private func deleteButtonTapped() {
        guard let no = discount?.no else { return }
        let url = "\(Bundle.main.TEST_URL)/discount/del/\(no)" // 실제 요청할 서버의 URL로 변경해주세요.
    ...
    @objc private func notificationButtonTapped() {
        guard let no = discount?.no else { return }
        let url = "\(Bundle.main.TEST_URL)/discount/update/notification/\(no)" // 실제 요청할 서버의 URL로 변경해주세요.
        AF.request(url, method: .put).response { response in
            switch response.result {
            case .success:
...
extension UserDiscountDetailVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "UserDiscountDetailCell", for: indexPath) as! UserDiscountDetailCell
        guard let discount = discount, let imageList = discount.imageList else {
            return cell
        }
        cell.setImages(imageList)
        cell.setText(it: discount)
