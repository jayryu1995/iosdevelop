...
class CollabWriteVC: UIViewController, UIScrollViewDelegate, UINavigationControllerDelegate {
    ...
    private let tv_title: UITextView = {
    ...
    private func setBottomView() {
        ...
        styleLabel.snp.makeConstraints{
            $0.top.equalTo(tf_people.snp.bottom).offset(20)
            $0.leading.equalToSuperview().offset(20)
            $0.trailing.equalTo(view).offset(-20)
        }
    ...
    private func setupContentTextView() {
        ...
        tv_title.snp.makeConstraints { make in
            ...
            make.height.equalTo(48)  // 두 줄에 해당하는 높이로 고정
        }
    ...
    @objc private func snsButtonTapped(_ sender: UIButton) {
        ...
        if let index = selectedSNS.firstIndex(of: sns.localized) {
            ...
            // 선택되지않은 버튼 동작
            selectedSNS.append(sns.localized)
            sender.backgroundColor = .black
            sender.setTitleColor(.white, for: .normal)
            sender.isSelected = true
        }
    ...
    @objc private func nationButtonTapped(_ sender: UIButton) {
        ...
        // 현재 선택된 스타일이 다시 탭된 경우, 선택 해제하지 않고 상태를 유지합니다.
        if selectedNation != sender.tag.toString() {
            selectedNation = sender.tag.toString()
            sender.backgroundColor = .black
            sender.setTitleColor(.white, for: .normal)
            sender.isSelected = true
        } else {
    ...
    private func updateSNSSelectionUI() {
        snsButtons.forEach { button in
            if let sns = button.titleLabel?.text {
                if selectedSNS.contains(sns.localized) {
...
extension CollabWriteVC: UIImagePickerControllerDelegate, UITextViewDelegate, UITextFieldDelegate {
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        ...
        // 최대 3000자까지만 입력 가능하도록 설정
        if updatedText.count > 3000 {
            return false
        }

        // 줄 수 제한은 tv_title에 대해서만 적용
        if textView == tv_title {
            let numberOfLines = updatedText.components(separatedBy: "\n").count
            if numberOfLines > 2 {
                return false
            }
        }
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        if textView == tv_title && textView.text == "collabWriteVC_title".localized {
                textView.text = ""
    ...
    func textViewDidEndEditing(_ textView: UITextView) {
        if textView == tv_title && textView.text.isEmpty {
                textView.text = "collabWriteVC_title".localized
                textView.textColor = .lightGray
            }
