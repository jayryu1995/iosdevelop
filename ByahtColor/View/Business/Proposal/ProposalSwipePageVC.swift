...
class ProposalSwipePageVC: UIViewController {
    ...
    private func setupUI() {
        ...
        deleteButton.addTarget(self, action: #selector(deleteProposal), for: .touchUpInside)
    ...
    private func setupConstraints() {
        ...
        chatButton.snp.makeConstraints {
            $0.trailing.equalToSuperview().inset(10)
            $0.bottom.equalTo(deleteButton.snp.top).inset(20)
        }
    ...
    @objc private func navigationToChats(_ sender: UIButton) {
        ...
        if let memberId = self.profile?.memberId {
            self.viewModel.deleteProposal(member_id: memberId) { [weak self] result in
                switch result {
                case .success:
                    print("업데이트 성공")
                    self?.delegate?.didUpdateProposalList(memberId:memberId)
    ...
    @objc private func deleteProposal(_ sender: UIButton) {
        showProposalAlert()
    }
    
    private func showProposalAlert() {
        let proposalAlertVC = BusinessProposalAlert2VC()
